...
public class BookingMangingLoginScreenConrtroller extends AbstractScreen{
    ...
    void openBookingManagingScreen(ActionEvent event) {
    	...
    	// validating IDNumberTextField contains only numbers:
    	 ...
    	 {
     		showErrorAlert(ScreenManager.getInstance().getStage(),"Please make sure that the ID number contains only numbers");
     		IDNumberTextField.setStyle(setFieldToError()); 
    	 }
    	// validating IDNumberTextField contains exactly 9 numbers:
    	else if (idNumber.matches("\\d{1,8}"))
    	 {
      		showErrorAlert(ScreenManager.getInstance().getStage(),"The ID you entered has less than 9 digits. Please ensure your ID is exactly 9 digits long");
      		IDNumberTextField.setStyle(setFieldToError()); 
    	 }
    	else if (idNumber.matches("\\d{10,}"))
    	 {
      		showErrorAlert(ScreenManager.getInstance().getStage(),"The ID you entered has more than 9 digits. Please ensure your ID is exactly 9 digits long");
      		IDNumberTextField.setStyle(setFieldToError()); 
    	 }
      	 else
      	 {
			...
			/////////////////////////////////////////	
 			...
 			//false, false,StageSettings.defaultSettings("GoNature System - Reservations"), pair);   
      	 }
    }
