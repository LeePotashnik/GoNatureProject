...
public class VisitReportController extends AbstractScreen {
	...
	/**
	 * Populates a scatter chart with data for group visits and individual visits. The method expects a map
	 * where keys are strings representing the type of visit ("Group" or "Individual Visitor") and values are lists
	 * of XYChart.Data objects, with each object representing a data point for the chart.
	 *
	 * @param chartData A map containing the data to be displayed on the chart. The keys should be
	 *                  "Group" and "Individual Visitor", and the values should be lists of XYChart.Data objects.
	 */
	public void populateChart(Map<String, List<XYChart.Data<Number, Number>>> chartData) {
	    ...
	    List<XYChart.Data<Number, Number>> individualVisits = chartData.getOrDefault("Individual Visitor", new ArrayList<>());
