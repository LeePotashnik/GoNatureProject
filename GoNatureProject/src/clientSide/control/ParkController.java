...
/**
 * The ParkController class serves as a controller for managing park-related operations within the application.
 * It provides functionalities to interact with park data, including fetching park details from the database,
 * managing bookings, checking visitor and booking existence, updating park capacities, and handling visitor bookings.
 * This class follows the Singleton pattern to ensure that only one instance of the controller is used throughout
 * the application, providing a centralized point of access to the park data and operations.
 */
public class ParkController {
	 ...
	 * Initializes the controller instance. This approach is part of the Singleton pattern,
	 * ensuring that ParkController is only instantiated once throughout the application life cycle.
	 */
	...
	/**
	 * Restores the previously saved Park object from the controller's state. This method is typically
	 * used to retrieve the park information that was last set or modified within the application session.
	 * 
	...
	/**
	 * Saves a Park object to the controller's state. This method is typically used to store the park
	 * information that needs to be retained across different operations within the application session.
	 * 
	...
	/**
	 * Generates a database-friendly table name from a park name by converting it to lower case
	...
	/**
	 * Converts a database table name back to a more readable park name by replacing underscores with spaces
	 * and capitalizing the first letter of each word.
	 * 
	 * @param park The string representing the database table name to convert back to a park name.
     ...
     * @return A ParkVisitor object if the visitor exists, null otherwise. 
     ...
     * In case the maneger is Park Manager only one park will retrieve from the DB
     * @param park
     * @return 
     ...
     * In case visitors only arrive at the park, the currentCapacity will increase. Otherwise, the opposite.
    ...
    /**
     * Retrieves current capacity and limits information for a specified park from the database. 
     * This method executes a 'SELECT' SQL query to obtain details such as the maximum visitors capacity,
     *  maximum order amount, maximum time limit, and the current capacity of the park.
     * @return 
     * 	A String array containing four elements:
     *   	1. Maximum visitors capacity (retValue[0])
     *      2. Maximum order amount (retValue[1])
     *      3. Maximum time limit for visits (retValue[2])
     *      4. Current capacity of the park (retValue[3])
    ...
    /**
     * Inserts a new booking record into a specified table within the database, based on the type of the booking.
     * This method supports inserting bookings into tables designated for active bookings, completed bookings, 
     * or cancelled bookings. The table into which the booking is inserted is determined by the combination of the 
     * park's name and the specified booking type (active, done, or canceled).
     * Depending on the booking type, different sets of booking details are included in the insert query.
     *
     * @param newBooking The new Booking object containing the details of the booking to be inserted.
     * @param relevantTable A suffix indicating the specific table to insert the booking into (e.g., "_park_active_booking").
     * @param type The type of the booking, determining which table the booking is inserted into ("active", "done", "canceled").
     * @return true if the insert operation is successful and the booking is correctly added to the database; false otherwise.
	 */
